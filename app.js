...
window.loadWeek = function() {
  ...
  ['basico', 'alto', 'superior'].forEach(nivel => {
    ...
    const t = data.teoria[nivel]; const claveReclamada = `teoria_leida_${currentRetoId}_${nivel}`; const yaReclamado = userData.teoria[claveReclamada] === true;
    const tarjeta = document.createElement('div'); tarjeta.className = `teoria-card border-${nivel}`;
    tarjeta.innerHTML = `<div class="teoria-header"><h4><i data-lucide="book-open"></i> ${t.titulo}</h4><span class="recompensa-badge">🪙 +${t.monedas}</span></div><p class="teoria-texto">${t.contenido}</p><div class="teoria-ejemplo">${t.ejemplo}</div><button id="btn-teoria-${nivel}" class="btn-reclamar ${yaReclamado ? 'reclamado' : ''}" onclick="window.reclamarMonedas('${currentRetoId}', '${nivel}', ${t.monedas})">${yaReclamado ? '✔️ Recompensa Reclamada' : 'Marcar como leído y ganar 🪙'}</button>`;
